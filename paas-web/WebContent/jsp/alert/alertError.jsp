	...
	<div class="alert">
		<div class="alert-main">
			<div class="alert-main-title">
				<h1><img src="${ctx}/imgs/runtime-error.png" width="29" height="30" style="margin-bottom:-5px; margin-right:8px;"/>操作失败！</h1>
				<span>
			...
			<% if(request.getParameter("msg").contains("?appIds")){session.setAttribute("token",  "&token="+request.getParameter("token"));}else{session.setAttribute("token",  "");} %>
				【<%=request.getParameter("code")%>】<%=request.getParameter("msg")%><%=request.getSession().getAttribute("token")%>
			</div>
