			...
			.jqGrid(
					{
						...
						url : '/paas/log/queryAppLog.action',
						datatype : "json",
						width: "100%",
						autowidth:true,
						height : "100%",
						...
						colModel : [
								{
									...
									formatter : function(val, options,
										...
										if ("1" == rowObject["fileType"]) {
											var _link = '<a href="appServiceLog.jsp?subPath='
													+ rowObject["filePath"]
													+ "/"
													+ rowObject["fileName"]
													+ '&id='
									...
									formatter : function(val, options,
										...
										if ("0" == rowObject["fileType"]) {
											...
											_link = '<a href="/paas/log/downloadFile.action?downloadPath='
													+ val
													+ '&serviceId='+serviceId+'" style="color:#666">'
...
$(function(){
    $(window).resize(function(){   
    	$("#appLogList").setGridWidth($(window).width()*0.99);
});
});
...
function getPath() {
	...
	$.ajax({
		...
		url : "/paas/log/queryLogPath.action",
		success : function(result) {
			if (result['resultCode'] == 'success') {
				var json = eval(result['resultMessage']);
				$('#path').text(json);
