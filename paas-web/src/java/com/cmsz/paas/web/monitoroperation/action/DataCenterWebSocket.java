...
@WebListener
@ServerEndpoint(value = "/websocket/dataCenter", configurator = GetHttpSessionConfigurator.class)
public class DataCenterWebSocket {

	/** The Constant logger. */
	private static final Logger logger = LoggerFactory.getLogger(DataCenterWebSocket.class);
...
//	private final String TEMP_FILE_PATH = "tempfile/";
	...
	public void onMessage(String message, Session session) {
		Log.info("同步数据中心长连接收到的请求消息：" + message);
		String dataCenterId = "";
		if(message.indexOf("|")>-1) {
			dataCenterId = message.split("\\|")[0];
		}else {
			dataCenterId = message.split("%7C")[0];
		}
		BufferedReader reader = null;
		ResponseInfo responseInfo = null;
		try {
			//组装访问控制中心的链接，以及参数
			String url = RestUtils.restUrlTransform("syncDataCenterUrl", dataCenterId);
			logger.debug("同步数据中心rest接口：" + url);
			...
			while((currentLine = reader.readLine()) != null){
				...
				logger.info("同步数据中心rest接口返回码：" + retCode + ", 返回信息：" + msg);
			}
