...
@WebListener
@ServerEndpoint(value = "/websocket/privateImage", configurator = GetHttpSessionConfigurator.class)
public class PrivateImageWebSocket {

	/** The Constant logger. */
	private static final Logger logger = LoggerFactory
			.getLogger(PrivateImageWebSocket.class);
...
//	private final String TEMP_FILE_PATH = "tempfile/";
	...
	public void onMessage(String message, Session session) {
		...
		String privateImageVersionId = "";
		String version = "";
		if(message.indexOf("|") > -1) {
			privateImageId = message.split("\\|")[0];
			privateImageVersionId = message.split("\\|")[1];
			version = message.split("\\|")[2];
		}else {
			privateImageId = message.split("%7C")[0];
			privateImageVersionId = message.split("%7C")[1];
			version = message.split("%7C")[2];
		}
