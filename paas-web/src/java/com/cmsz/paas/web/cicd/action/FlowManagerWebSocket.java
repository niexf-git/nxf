 ...
 * @date 2016-4-12
 */
@ServerEndpoint(value = "/websocket/stepLog", configurator = GetHttpSessionConfigurator.class)
public class FlowManagerWebSocket {
	
	private static final Logger logger = LoggerFactory.getLogger(FlowManagerWebSocket.class);
	
	private StepDetailDao stepDetailDao = Springs.getBean("stepDetailDao");
	...
	public void onMessage(String message, Session session) throws IOException,
		...
		try {
			if (StringUtils.isNotBlank(message)) {
				...
				String stepName = split[2];
				String restUrl = RestUtils.restUrlTransform("queryExcuteRecordLogsUrl", flowId, stepId,stepName);
	...
	private void outputBuildLog(String restUrl, Session session) throws IOException, JSONException{
		logger.info("开始调用Rest接口查询单步骤构建记录日志:" + restUrl);
		try{
			String currentLine = null;
			reader = stepDetailDao.get(restUrl);      //此处控制中心还没有get方法的长连接
			...
			logger.info("查询单步骤构建日志错误", ex);
			responseInfo.setRetCode(Constants.QUERY_STEP_LOGS_ERROR);
	...
	private String sendResult(ResponseInfo responseInfo) throws JSONException {
		...
		if(!Constants.REST_CODE_SUCCESS.equals(retCode)) {
			...
			if(retCode.contains("PAAS-00")) {
				...
				logger.error("查询单步骤构建日志错误", resultMessage);
			}
		}else {//成功
			resultMessage = "查询单步骤构建日志完成！";
