...
import com.cmsz.paas.common.lang.Assert;
...
public class Files extends FileUtils {
	...
	public static File createTxtFile(File path, String content) {
		if (path == null)
			throw Lang
					.makeThrow("Can not create a txt file,becaue the path is Null!");
		...
		try {
			...
			throw Lang.wrapThrow(e, "Create txt file[%s] is error!",
	 ...
	 * @throws IOException
	 */
	public static void changeEncoding(File file, String srcEncode,
			String targetEncode) throws IOException {
	 ...
	 *            指定要装载的文件
	 * @param fileEncode
	 *            指定的文件编码,如果是以GBK进行存储的话，读取也要以GBK进行读取，否则会乱码
	 * @return "" 表示指定的路径不存在或者不是一个文件
	 * @throws IOException
	 */
	public static String loadTxtFile(File file, String fileEncode)
			throws IOException {
	...
	public static Properties loadPropertiesFile(String loadPath) {
		...
		try {
			prop.load(Thread.currentThread().getContextClassLoader()
					.getResourceAsStream(loadPath));
		} catch (IOException e) {
			Lang.wrapThrow(e,
					"load properties file['%s'] is error,cause is:'%s'",
					loadPath, e.getMessage());
		}
		return prop;
	}
	...
	public static String toString(File f) {
		...
		if (f == null)
			return "";
		return String.format("[Path:%s,Name:%s,FileOrDiectory:%s]", f
				.getAbsolutePath(), f.getName(), f.isDirectory() ? "Directory"
				: "File");
	}
	...
	public static void zips(File src, File target) {
		...
		try {
			...
			Lang.wrapThrow(e, "file[%s] zip to file[%s] is error!",
	...
	private static void zips(File base, File currentZipFile,
	...
	public static void unzip(File srcZipFile, File targetPath) {
		...
		if (targetPath.isFile())
			throw Lang
					.makeThrow("target unzip path can not be a file,must be a directory!");
		...
		try {
			...
			throw Lang.wrapThrow(e, "unzip file[%s] to file[%s] is error!",
					srcZipFile.getAbsolutePath(), targetPath.getAbsolutePath());
		}
	...
	public static void main(String[] a) throws IOException {
		...
		// };
		// String dir =
		// "E:/dev/project/IBMS_PROJECT/projects/ibms/webapp/jsp/common";
		...
		// Files.changeEncoding(f, Strings.ENCODING_GBK,
		// Strings.ENCODING_UTF_8);
		// String strEc = f.getAbsolutePath().replace("\\", "/");
		// strEc =
		// strEc.replaceAll("E:/dev/project/IBMS_PROJECT/projects/ibms/webapp/jsp",
		// ecdir);
		// strEc += "=UTF-8";
		// sb2.append(strEc).append(Strings.NEW_LINE);
		// strEc = "";
		// } catch (Exception e) {
		// sb.append(String.format("处理失败！文件：%s,路径:%s", f.getName(),
		// f.getParent())).append(
		// Strings.NEW_LINE);
		// }
		// }
		...
		// Lang.println(sb2);
		// String f =
		// "E:/dev/project/IBMS_PROJECT/projects/ibms/webapp/js/forecast_common.js";
		// Files.changeEncoding(new File(f),Strings.ENCODING_GBK,
		// Strings.ENCODING_UTF_8);
