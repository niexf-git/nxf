...
public abstract class Maps extends org.apache.commons.collections.MapUtils {
	 ...
	 * @return
	 */
	public static <K, V> Map<K, V> map(Class<K> kClass, Class<V> vClass,
			Object... keyValues) {
	...
	/**
	 * 将某个List转成Map，转换规则：key为List中的item的某个属性为,value为List中的item
	 * 
	 * @param <K>
	 *            生成的map的key类型
	 * @param <V>
	 ...
	 *            要转成map的list
	 * @param property
	 *            list中的item属性，要指定为map key的属性
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static <K, V> Map<K, V> of(List<V> values, String property) {

		Map<K, V> map = newMap();
		if (Collections.isNotEmpty(values)) {
			for (V v : values) {
				K key = (K) Beans.getAnyProperty(v, property);
				map.put(key, v);
			}
		}
		return map;
