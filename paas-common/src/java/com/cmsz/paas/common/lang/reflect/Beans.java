...
package com.cmsz.paas.common.lang.reflect;
 ...
 * @author sam,kevin
 */
public abstract class Beans extends org.apache.commons.beanutils.BeanUtils {
	...
	public static void setProperty(Object o, String p, Object v) {
		...
		try {
			...
			throw Lang.wrapThrow(e, "设置对象(类型：%s)属性\"%s\"时出错了！！", o.getClass()
					.getName(), p);
		}
	}
	...
	// } catch (Exception e) {
	// throw Lang.wrapThrow(e, "根据Map[%s]设置对象[%s]的值出错了！！", properties,
	// bean.getClass()
	// .getName());
	// }
	// }
	 ...
	 * @return
	 */
	// public static <T> T setProperties(T t, Map<?, ?> properties) {
	//
	...
	public static String convertListToIn(List list, String fieldName) {
		...
		for (int i = 0; i < list.size() - 1; i++) {
			sb.append("'").append(getFieldValue(list.get(i), methodName))
					.append("',");
		}
		sb.append("'")
				.append(list.size() > 0 ? getFieldValue(
						list.get(list.size() - 1), methodName) : "")
				.append("'");
		sb.append(")");
		return sb.toString();
	}
	...
	/**
	 * 
	 * <b>功能描述：</b>取值属性值，如果对象没有相应的属性，则制造对应的方法，不一定需要get,或is开头
	 * 
	 * @author ：谢艳华（Kevin.xie） <br>
	 *         <b>创建日期 ：</b>2011-1-2 下午11:54:29
	 * 
	 * @param obj
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Object getAnyProperty(final Object obj,
			String propertyExpression) {

		if (obj == null || propertyExpression == null
				|| propertyExpression.trim().length() < 1) {
		...
		for (int i = 0; i < names.length; i++) {
			...
			if ((result instanceof Collection)) {
				...
				try {
					...
					Lang.wrapThrow(e, "get array property from [%s]", obj
	...
	public static Object getSimpleValue(Object obj, String property) {
		...
		try {
			...
			try {
				result = MethodUtils.invokeExactMethod(obj, property,
						new Object[] {});
			} catch (Throwable ex) {
				try {
					...
					Lang.wrapThrow(exx, "getField %s.%s error!", obj.getClass()
	...
	public static void main(String[] s) {
		...
		BB bb = new BB();
		bb.aa.aa = "aaaa";
		Lang.println(getAnyProperty(bb, "aa.aa"));
	}
}
